...
describe Cohort do
  ...
  describe "#title" do
    ...
    it "does not have an attribute writer for title" do
      expect { cohort.title = "Cohort 30" }.to raise_error(NoMethodError)
    end
  end
  ...
  describe "#career_kickoff" do
    ...
    it "should be different for the next cohort" do
      ...
      philly5 = Cohort.new("Cohort 40", start_date, end_date)

      expect(philly5.career_kickoff).to eq(Date.parse("2020/08/09"))
    end
  end
